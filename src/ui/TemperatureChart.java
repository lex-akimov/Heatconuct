...
class TemperatureChart extends JPanel implements KeyListener {
    ...
    static boolean drawLayerBound = true;
    static boolean drawDegreeLines = true;

    TemperatureChart(float min, float max) {
        this.layers = new float[Window.parser.getTemperaturesArray()[0].length - 2];
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(i = 1; i < this.layers.length; ++i) {
            ...
            g2d.drawLine(x, 520 - (int)((Window.parser.getTemperaturesArray()[Window.currChart][i] - (float)this.min) * 500.0F / (float)(this.max - this.min)), xx, 520 - (int)((Window.parser.getTemperaturesArray()[Window.currChart][i + 1] - (float)this.min) * 500.0F / (float)(this.max - this.min)));
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getSource() instanceof JTextField && e.getKeyChar() == '\n') {
            int i;
            for(i = 1; i < this.layers.length; ++i) {
                this.layers[i] = this.layers[i - 1] + (float)Integer.parseInt(((JTextField)this.thicknessByLayers.get(i - 1)).getText());
            }
