...
import static ui.CONSTANTS.CHART_TITLE;
...
class MainPane extends JPanel {
    ...
    private class RightPan extends JPanel {
        ...
        public void repaint() {
            if (Tout != null) {
                Tout.setText(String.format("tнар. = %+2.1f ˚C", Window.parser.getTemperaturesArray()[Window.currChart][Parser.temperaturesCount - 1]));
            }

            if (AlphaOut != null) {
                AlphaOut.repaint(Window.parser.getAlphaOut()[Window.currChart]);
            }

            if (QOut != null) {
                QOut.repaint(Window.parser.getQOut()[Window.currChart]);
            }
        }
    }
    ...
    private class LeftPan extends JPanel {
        ...
        public void repaint() {
            if (dateTimeLabel != null) {
                dateTimeLabel.setText(MainPane.dateTime.format(Window.parser.getDatesArr()[Window.currChart]));
            }

            if (Tin != null) {
                Tin.setText(String.format("tвн. = %+2.1f ˚C", Window.parser.getTemperaturesArray()[Window.currChart][0]));
            }

            if (AlphaIn != null) {
                AlphaIn.repaint(Window.parser.getAlphaIn()[Window.currChart]);
            }

            if (QIn != null) {
                QIn.repaint(Window.parser.getQIn()[Window.currChart]);
            }
        }
    }
}
