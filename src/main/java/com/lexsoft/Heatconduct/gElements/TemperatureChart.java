...
public class TemperatureChart extends JPanel implements KeyListener {
    ...
    public TemperatureChart(float min, float max) {
        ...
        for(int i = 1; i < length; ++i) {
            int x = (int)(199.0F * (this.layers[i - 1] + this.layers[i]) / this.layers[length - 1] + 45.0F);
        ...
        this.thickness = new JLabel(String.format("%.0f", this.layers[length - 1]));
    ...
    protected void paintComponent(Graphics g) {
        ...
        float one = 398.0F / this.layers[length - 1];


        for(int i = 1; i < length; ++i) {
            int x = (int)(45.0F + this.layers[i - 1] * one);
        ...
        for(int i = this.min; i <= this.max; i += 5) {
        ...
        for(int i = 0; i < length; ++i) {
            int x = (int)(45.0F + this.layers[i] * one);
            if (i != 0 && i != length - 1) {
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getSource() instanceof JTextField && e.getKeyChar() == '\n') {
            ...
            this.thickness.setText(String.format("%.0f", this.layers[length - 1]));
            this.repaint();
        }
